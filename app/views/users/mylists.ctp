...
foreach($lists as $list){
    ...
    ?>
    <div class="thread">
        <div  id="list<?php echo $list['Mylist']['id'];?>" style="width:50%; display:block; float:left"><?php echo $list['Mylist']['name'];?></div>
<?php
    echo $ajax->editor( "list".$list['Mylist']['id'],
                        array(
                            'controller' => 'mylists',
                            'action' => 'updatelistname', $list['Mylist']['id']
                        ),
                        array(
                            'indicator' => '<img src="../img/loading.gif">',
                            'submit' => __d('logged', 'change',true),
                            'style' => 'inherit',
                            'submitdata' => array('id'=> $list['Mylist']['id']),
                            'tooltip'   => 'Click to edit...'
                            )
        );
?>
        <div id="remove" style="float:right; margin-left:5px; display:block; width:5%">
        <?php
